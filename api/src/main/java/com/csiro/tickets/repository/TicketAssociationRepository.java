...
public interface TicketAssociationRepository extends JpaRepository<TicketAssociation, Long> {
  ...
  @Query("SELECT ta FROM TicketAssociation ta WHERE (ta.associationSource = :source AND ta.associationTarget = :target) OR (ta.associationSource = :target AND ta.associationTarget = :source)")
  List<TicketAssociation> findBySourceAndTargetFlipped(@Param("source") Ticket source, @Param("target") Ticket target);
}
