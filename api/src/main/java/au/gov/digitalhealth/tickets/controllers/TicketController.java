...
import static au.gov.digitalhealth.tickets.models.AdditionalFieldType.ADDITIONAL_FIELD_TYPE_NAME_PATTERN;
...
public class TicketController {
  ...
  private static final Pattern ADDITIONAL_FIELD_TYPE_NAME_REGEX =
  ...
  public ResponseEntity<List<TicketMinimalDto>> searchByAdditionalFieldTypeNameValueOfList(
    ...
    if (!ADDITIONAL_FIELD_TYPE_NAME_REGEX.matcher(additionalFieldTypeName).matches()) {
      throw new LingoProblem(
          "ticket-search",
          "Invalid Additional Field Type Name specified",
          HttpStatus.BAD_REQUEST,
          "Additional Field Type Name must be from 1 to 30 upper or lower case characters with no spaces or special characters");
    }

    if (valueOfs.stream()
        .anyMatch(
            value ->
                !org.apache.commons.lang3.StringUtils.isAsciiPrintable(value)
                    || value.length() > 30)) {
      throw new LingoProblem(
          "ticket-search",
          "Invalid field value specified",
          HttpStatus.BAD_REQUEST,
          "Field value must be less than 30 ordinary ASCII characters");
    }
