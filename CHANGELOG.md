...
## [Unreleased]
- Fixed a product save failing against a locked branch being reported to the author as a fatal 500 server error. `SnowstormClient.waitForBranchLock` already waits out a lock that is present when a request starts, and gives up with a 423 LOCKED after 10 seconds, but the branch can be locked in the window between that check passing and Snowstorm beginning the bulk change — a classification, promotion or rebase starting on the task is enough. Snowstorm reports that as a failed batch whose *message* is "Branch ... is already locked", and it was surfacing as a `BatchSnowstormRequestFailedProblem` (500), which is both wrong — the branch being busy is an expected transient condition, not a server fault — and unactionable, since the author got a generic server error. A batch that fails on a branch lock is now a `BranchLockedProblem` (423) naming the branch and saying what to wait for. Because 423 is a 4xx, the existing `ClientErrorNoiseFilter` stops it reaching Sentry at all, where it had been landing as an unhandled fatal. The message also tells the author what it means for their data: the failed batch itself wrote nothing, because Snowstorm takes the branch lock before applying any content, but a single save issues several independent writes — property-only updates in one batch, created concepts in another, then refset memberships, then the ticket update, with a blob-storage upload before any of them — so an earlier step that already succeeded is not rolled back, and the product should be re-opened before retrying. `BranchLockedProblem` now carries guidance in all three places it is raised. Also stopped a failed batch that reports no concept ids throwing a `NullPointerException` from `Objects.requireNonNull` in place of Snowstorm's actual failure message — which is exactly the shape a batch that failed before doing any work returns. (IEDC-9351)
- Renamed the `Nutritional` option in the Target Model dropdown to `Nutritional/Consumer Health`, as requested by HSE.
- ARTG ticket content is now composed in one place — submission-gateway — rather than here and in the TGA feed processor separately, and this application accepts the whole of it rather than a name and a description. `TicketMetadata` gained the state, schedule, priority bucket, label removals, register snapshot, additional field values and changed-flags a caller resolved, so a caller that had composed that content no longer has to create the ticket and then patch it — which cost a second write and left a window in which the ticket existed without it. Updates from such a caller are now symmetric with creation: an update could previously only ever *add*, so a label derived from register content stayed on the ticket after the content stopped justifying it, and a composed description was filed as a comment instead of becoming the description. Callers that submit a bare identifier are unchanged — they composed nothing, so their description really is an addendum. Additional field values now follow the caller's order, which is the display order (ARTG ID, start date, effective date for ARTG tickets); `Ticket.additionalFieldValues` and the two shared paths that build them were `HashSet`s, so the order reaching the join table came out by hash — this affects all ticket creation, not only ARTG. The `ARTGID` dedupe stamp is merged into the caller's fields rather than appended, because a ticket whose stamp was *replaced* would be invisible to the very lookup that created it. Creation now also claims the work identity first, closing a race in which two callers submitting the same ARTG ID both found nothing and both created a ticket. `SergioService`, the `sergioApiClient` bean, `sergio.base.url` and the `processArtgId` method that called them are removed, which ends an availability inversion: this application could not create a ticket by ARTG ID while a nightly batch job was unavailable. The ticket is still created here, so a caller's external requestors, labels and description addendum are merged exactly as before. Two cautions for whoever touches this next: `TicketMetadata` exists as two classes, one here and one in submission-gateway, matched only by JSON field name — a name that differs on one side is silently dropped rather than rejected, so they must change together; and **deployments must set `submissiongateway.base.url` and the `submissiongateway.oauth2.*` client credentials issued to this application, because there is no local fallback and ARTG ticket creation fails without them.** (#1980)
- Bumped every `@tiptap/*` package from 3.10.1 to 3.30.5, clearing GHSA-j95f-988m-3j2f, a HIGH quadratic ReDoS in Tiptap's block and inline Markdown attribute parsing. `@tiptap/core` is reached both directly and through `mui-tiptap`, so the three `pnpm.overrides` entries (`@tiptap/core`, `@tiptap/pm`, `@tiptap/react`) were moved in step with the 35 direct dependencies — pinning only the direct ones would have left the copy `mui-tiptap` resolves untouched. The editor is used for ticket descriptions and comments, which accept pasted content, so the parser is reachable with attacker-influenced input. Every pull request was failing the `Dependency scan (Trivy)` job on this regardless of what the branch changed. (#1979)
- No updates yet.


## [1.4.8] - 2026-09-22
- Fixed the atomic screen loading a product as a blank form — no data, no spinner and no error — when it was opened straight after viewing a product in view mode. The authoring screens wrote their form state from *inside* the React Query `queryFn`, via a `setFunction` callback, so the state setters that ran belonged to whichever component instance created the query rather than the one currently mounted. React Query executes one `queryFn` per query key and dedupes every other observer onto it: opening a product in view mode unmounted the authoring screen while its request for the same key was still in flight, the next authoring instance subscribed to that existing query, `queryFn` never ran again, and the setters that fired belonged to the unmounted instance. The response arrived and every write was silently discarded. Results are now applied from the query's `data` in an effect, so each observer populates its own state from the result it receives and it no longer matters which instance performed the fetch. Clicking a different product always worked because it used a different key and so ran its own `queryFn`, which is why this only ever reproduced against a product that had just been loaded. A failed load is also reported now instead of rendering an empty form — the hook previously had no error path at all, so a 404 on a stale row was indistinguishable from a successful load of an empty product. `useTicketProductQuery` and `useTicketProductAuditQuery` additionally include the ticket id in their query keys, which they previously omitted. The mount-time form reset now runs as a layout effect so that it is guaranteed to precede the effects that apply a loaded product: those are declared above it, and applying a result during mount rather than asynchronously after it means a passive reset would run last and wipe what they had just written.
- Fixed every concept Snomio creates being persisted as Primitive regardless of how its axiom was modelled — reported as VMPP and AMPP showing as Primitive, with failed assertions in the validation report, after a product's modelling was edited in the atomic screen and re-classified (a Sona Vitamin, and a Sodium Chloride 30% injection where a strength was changed from milligram to gram). `SnowstormDtoUtil.toSnowstormConceptView` derived the concept-level `definitionStatusId` by comparing the SCTID `DEFINED.getValue()` (`900000000000073002`) against the axiom's `definitionStatus`, which holds a name (`FULLY_DEFINED`) rather than an SCTID. The two fields carry different vocabularies, so the comparison could never match and PRIMITIVE was written every time. It went unnoticed for so long because the create response was built from the axiom, not from what was stored: `ProductCreationService` copies the definition status back onto the returned concept from the axiom, which was correct all along, so a concept looked Fully Defined immediately after creation and only showed as Primitive on reload. The comparison now reads the axiom's `definitionStatusId`. Concepts created before this fix carry the wrong definition status in the terminology server and are not corrected retrospectively.
- Corrected the atomic data extraction log messages that misreport an absent relationship as a duplicate one. `MedicationService` and `DeviceService` both logged "There are more than one relationship found for unbranded product" from the `else` of a `size() == 1` check, so a product with *no* `Has Other Identifying Information` — which is normal, most products carry none — was reported at SEVERE as having too many, naming neither the product nor the count. `AtomicDataService` had the same shape. Absence is now logged at FINE and only a genuine count mismatch is logged at SEVERE, in both cases naming the product and the actual count. The "Expected 1 active ingredient for precise ingredient ..." extraction failure also interpolated a joined list of concept ids that rendered as an empty string when nothing was found, producing "but found  for product ..." — it now reports the count, listing the ids only when there are some. These messages sent an investigation of a genuine multi-component pack failure down the wrong path for some time.
- Fixed loading a multi-component pack in the atomic screen failing with "Expected 1 active ingredient for precise ingredient ... for product ...". A pack whose contained products sit under different MP-only (VTM) concepts — Pabrinex Intravenous High Potency, whose two ampoules are modelled under `988791000220107` (ascorbic acid, nicotinamide, glucose) and `988801000220108` (thiamine, riboflavin, pyridoxine) — contributes more than one concept at each model level. NMPC contained products state only precise active ingredients, so `MedicationService` recovers the active ingredient from the VTM, but it selected that VTM with `findFirst()` over the whole pack's type map (a `HashMap`, so an arbitrary choice) and never scoped the lookup to the product being processed. Whichever VTM won, the precise ingredients belonging to the other ampoule could not resolve and extraction failed for the whole pack. The MP-only and clinical drug concepts are now resolved per product by walking its stated IS-A links, falling back to the previous behaviour when that is inconclusive. The content was correctly modelled; no authoring change is needed. Note the error names an arbitrary one of the affected substances and reports the *package* id, which made it look like a modelling defect in that substance.
- Forced the transitive `fast-uri` dependency up to 3.1.6, clearing four HIGH severity findings that blocked the Trivy scan: CVE-2026-75899, CVE-2026-75931, CVE-2026-75975 and CVE-2026-76172, all server-side request forgery or host confusion via URI parsing. The `ui/pnpm-workspace.yaml` override already pinned `fast-uri` to `^3.1.5` for an earlier advisory, and 3.1.5 is itself affected; it is reached only transitively through `ajv`, whose own range (`^3.0.1`) already permits the patched version, so nothing else re-resolved — the lockfile diff is `fast-uri` alone.
- Fixed the quick filter on `/dashboard/tasks` dropping the most recently typed characters. `GridToolbarQuickFilter` holds the typed text in its own state and only commits it to the grid's filter model after a 500 ms debounce, so anything re-initialising that state reverts the box to the last committed value — losing exactly the characters typed since. Two things churned it on every render: `quickFilterParser` was an inline arrow in `TableHeaders`, and `GridToolbarQuickFilter` keys `updateSearchValue`, the debounced updater built from it, and the effect that syncs the input back from the filter model all off that function's identity; and `TasksList` passed fresh `slots`/`slotProps` object literals, so the toolbar churned whenever the list re-rendered, which it does on every tasks refetch (`useAllTasks` polls on a 60 second `refetchInterval`). The parser is now defined once at module scope and both grid props are memoised.
- Fixed every `@CacheEvict` on `CachingConfig` and the `@Cacheable` IMS auth-cookie lookup silently doing nothing. `CachingConfig implements CachingConfigurer` while overriding none of its methods, and Spring resolves `CachingConfigurer` beans while it is still building its caching/async infrastructure — so `CachingConfig` and, transitively, its whole constructor graph were instantiated before `BeanPostProcessor` registration finished. 220 beans were logged at startup as "not eligible for getting processed by all BeanPostProcessors", `CachingConfig` itself never received an AOP proxy (so `emptyUsersCache`, `emptyJiraUsersCache`, `refreshSnowstormStatusCache`, `refreshApStatusCache`, `refreshCompositeUnitCache` and `refreshUniNumeratorDenominatorCache` evicted nothing, leaving the snowstorm/AP status caches stale for up to 5 minutes instead of 60 seconds and the unit caches for up to 25 hours instead of 1 hour), and neither did `ImsService`, so `getDefaultCookie()` re-authenticated against IMS with username and password on every single call instead of reusing the cached cookie. Removing the no-op interface takes the warning count to zero and restores auto-proxying for all of them. A new `ProxiedAnnotationIntegrityTest` boots the real application context and fails the build if any bean declaring `@Cacheable`/`@CacheEvict`/`@CachePut`/`@Caching`/`@Async` is not actually proxied. (#1973, alternative to #1972)
...
- Bumped the `fast-uri` pnpm override from `^3.1.5` to `^3.1.6` (resolves to 3.1.7), clearing four HIGH server-side request forgery advisories against `fast-uri` 3.1.5: CVE-2026-75899 (repeated hostname percent-decoding), CVE-2026-75931 (host confusion via skipped IDN canonicalization), CVE-2026-75975 (malformed IPv6 normalization) and CVE-2026-76172 (URI parsing flaw enabling SSRF and redirects). It is a production dependency, reached through `@rjsf/validator-ajv8` -> `ajv` -> `fast-uri`. The override already existed and was pinned to a version that was patched when it was written; these advisories moved past it. Every pull request was failing the `Dependency scan (Trivy)` job on this until now, regardless of what the branch changed. (#1978)
- Fixed attachments whose filename contains a character above U+00FF — a bullet, em dash, curly quote or any non-Latin script — showing "Unsupported file type" instead of a preview in the ticket screen. `AttachmentController` built the `Content-Disposition` header by concatenating the raw filename into it, and HTTP/1.1 header values are written as ISO-8859-1: Tomcat silently drops the *entire* header rather than send a value it cannot encode. The download therefore arrived with no filename at all, and the viewer works out whether it can render a file from the filename extension, so it fell through to the unsupported-type message. The file itself was never affected — downloading it worked and the PDF opened fine, just saved as `download.pdf`, which is `file-saver`'s fallback when it is handed an empty name. Names like `CMI • Green Weekend - Jungle Cake • 15g.pdf` are common on attachments sourced from suppliers. The header is now built with Spring's `ContentDisposition`, which keeps the plain `filename` parameter inside ISO-8859-1 and carries the exact name in the RFC 5987 `filename*` parameter; the UI reads `filename*` in preference to `filename`, so the true name survives for both preview and download. Two further defences: an attachment with a blank filename is now served as `attachment-<id>` rather than nothing, and the viewer falls back to the response content type when no usable extension is available, so a missing filename can no longer by itself make a file unpreviewable. (#1991)
- The backlog's Due Date column filter gained "Is Not Blank" and "Is Blank" match modes. Filtering on a due date previously meant picking an exact date or a range, which is little help when the question is "which open tickets have a due date at all" and every ticket carries a different one; "Is Not Blank" answers it directly, and combines with a State filter to list the open tickets that are waiting to be completed. The "Export Backlog to CSV" report shares the same search, so the new modes narrow an export too. Also fixes two faults found in the same code: a saved search never restored its due date filter at all (the filter was written out but had no case to read it back), and restored a saved created-date filter to an Invalid Date because it was parsed as day-first when it is saved as an ISO datetime; and the Due Date and Created columns were rendered in the backlog grid even when a user-defined table excluded them, because their column-visibility check was wired into a calendar-mode conditional rather than guarding the column. (#1961)
- Bumped the `netty.version` override from `4.2.16.Final` to `4.2.17.Final`, clearing CVE-2026-75595 (GHSA-c4c3-7fpv-j4q5, CRITICAL): a fragmented TLS ClientHello defeats Netty's SNI routing and falls back to the default `SslContext`, so a connection can be routed to the wrong virtual host. Netty is a production dependency, reached transitively through `spring-boot-starter-reactor-netty` -> `reactor-netty-http`. The override already existed and was pinned to a version that was patched when it was written; this advisory, published on 2026-09-08, moved past it. Every pull request raised since then was failing the `Dependency scan (Trivy)` job on it regardless of what the branch changed, and because `snomio_build_test_publish` lists `trivy_scan` in its `dependsOn`, the integration tests were being skipped entirely rather than run and reported. (#1961)
- Bumped the pinned `@tiptap/*` packages from `3.10.1` to `3.30.5`, clearing GHSA-j95f-988m-3j2f (HIGH): quadratic ReDoS in Tiptap's block and inline Markdown attribute parsing, which affects `@tiptap/core` from 3.7.0 up to 3.30.5. Tiptap is the editor behind ticket comments (`CommentAuthoror`, `Toolbar`, `useExtensions`), so the comment box is where a regression would show. All 35 pinned packages move together because they are pinned to an exact version and Tiptap ships its extensions in lockstep; the jump stays inside major 3. Published on 2026-09-08, three hours after CVE-2026-75595 above, which is why clearing Netty alone did not turn the `Dependency scan (Trivy)` job green. (#1961)
...
### Fixed
- Fixed products being written to a task with relationships pointing at concepts that don't exist there, which then fails task validation in the authoring platform with an unhelpful "unexpected error validating concept prior to save" (CUST1752655, NMPC-005337). A product saved against one task and later re-associated to another replays its attribute targets — a marketing authorisation holder, say — verbatim from the saved data, so a target authored on the original task and not yet promoted simply isn't on the new one. The pre-save check only covered the product's own concepts, not the concepts those reference; it now checks both in the same lookup, and reports a missing reference distinctly from a missing product concept, since the fix is to re-select that field. Free text that never resolved to a concept now blocks the save too, rather than only colouring the field. The same assertion runs server-side before anything is written, covering direct API calls, bulk actions and imports.
- Fixed the "Concept does not exist in this branch" warning on ECL autocomplete fields never appearing. It was computed from the field's `options` list, which the effect immediately above it had just inserted the current value into (a fix for a typeahead false positive), so the condition could never be true for a loaded value — the true positive went with it. The warning is now driven by the batched pre-save existence check via the authoring store, so it costs no per-field lookups, and the field now renders in a real error state rather than only tinting its helper text.
- `ConceptService.getFilteredConceptIdsByBatches` no longer swallows a failed lookup and returns an empty list. Callers use the result to decide which concepts are absent from the branch, so a transient Snowstorm failure was reported to the user as every concept being missing rather than as an outage.
...
### Fixed
- Fixed the Branded product name field intermittently staying empty (or filling only after a long pause) when loading a robot-created partial product from a ticket (CUST1737896, IEDC-7474 follow-up). Drafts saved under an older schema carry stale oneOf discriminators (e.g. `variant=medication` with `productType=noIngredients`); correcting these through the form meant the first change flipped the oneOf branch and the ticket suggestion queued behind the resulting re-render storm on slower machines. Loaded ticket products are now normalised at the data level — stale discriminators coerced and the branded-product-name suggestion seeded — before the form mounts, with the widget retained as a backstop for late-arriving suggestions. Also fixed a silent dead state where a blank derived suggestion (possible via the PCRS `PackDescription` slice matching an empty product name) was treated as a real suggestion, seeding nothing and rendering no hint: the backend now treats a blank derivation as absent and the UI maps blank suggestions — and suggestion-fetch failures, previously swallowed silently — to the "Couldn't derive a name from the ticket" hint.
...
### Changed
- Improved atomic authoring form responsiveness on lower-spec machines (first tranche of #1932), measured live against a real NMPC draft: median per-keystroke main-thread cost dropped from ~555 ms to ~130 ms (dev build, unthrottled — roughly 4.5×, so seconds-per-keystroke at HSE-like CPU speeds becomes sub-second). The dominant fix: `validator.isValid` recompiled its AJV schema on every call (the `{ ...schema }` copy defeated AJV's identity-keyed cache), and RJSF calls it for every oneOf branch match on every change event and field mount — compiled validators are now cached by schema reference with a serialised-schema fallback. Also in this tranche: `resetDiscriminators` now runs a read-only dry-run first and returns its input untouched when no discriminator needs resetting (the overwhelmingly common case), eliminating a `cloneDeep` of the entire package details per change event plus a redundant re-render after ticket-product load, and keeping object identity stable for React; the RJSF `validator` prop is memoised in both authoring screens so RJSF stops rebuilding its `schemaUtils` (and losing its caches) every render; and `initializeOpenTelemetry()` is now idempotent — it was called at module scope from both `main.tsx` and `MainBody.tsx`, double-registering the zone.js user-interaction/fetch instrumentation and span processors. The remaining per-keystroke cost is the fully controlled RJSF form re-rendering the whole tree per change; that (semi-uncontrolled form, memoised templates, mount-cascade and lazy ECL fetches) is tracked as follow-up in #1932.
- Sentry now drops error reports caused purely by upstream (Snowstorm/authoring) *unavailability* — HTTP 502/503/504 responses and connection failures — in non-production environments, where those outages are outside our control and are pure noise. Real errors (other 4xx/5xx, exceptions, validation problems) still report in every environment, and all errors continue to report in production. The set of environments treated as production is configurable via `snomio.sentry.production-environments` (default `production,prod`) and the behaviour can be disabled with `snomio.sentry.suppress-non-prod-upstream-errors=false`.
- Sentry now also drops **client-disconnect** error reports — `ClientAbortException` / `AsyncRequestNotUsableException` ("connection reset by peer" / "broken pipe" raised when a browser aborts an in-flight request) — in every environment, since the caller is gone and the event carries no server-side signal. Disable with `snomio.sentry.suppress-client-disconnect-noise=false`.
- Sentry now also drops **expected client-error (4xx) reports** — not-found lookups, validation rejections, and duplicate/conflict responses that the service deliberately returns — in every environment, since they describe a bad or absent request from the caller rather than a server fault. 5xx server faults and upstream failures still report. Disable with `snomio.sentry.suppress-client-error-noise=false`.
...
### Fixed
- Fixed NMPC device authoring forcing the VTM to `63653004 |Biomedical device|`, which re-parented existing devices and proposed a new ATM on update (CUST1679268). The "VTM Level Device Type" is now selectable per the agreed bindings — VTM refset members that are devices (`(^660351000220100) AND (<<49062001)`, evaluable in the stated form the UI searches with), with the extended search widened to any device concept not already at another model level (`<<49062001 MINUS` the VMP/MPP/ATM/AMP refsets). The device product search and additional-parent bindings were widened from `<63653004` to `<49062001` to match. Loading a device product now also cross-checks the VTM resolved from the unbranded route (AMP→VMP→VTM) against the branded route (AMP→ATM→VTM) via a single cached ECL query, and rejects the product with an explicit data-remediation error naming both VTMs when the routes disagree (the Advance Catheter case), instead of silently authoring against the unbranded route's VTM and generating a duplicate ATM.
- Fixed the atomic authoring screen silently swallowing backend errors: a failed product `$calculate` (or any deliberate backend problem returned at a 5xx status) bounced the user back to the form with no explanation, or a Sentry crash-report dialog, instead of the error message. The axios response interceptor now surfaces the `detail` of any well-formed application problem (identified by the `http://lingo.csiro.au/problem/` type prefix) as a readable snackbar at any status, reserving the crash-report path for genuinely unexpected framework errors (CUST1730375, NMPC-001949).
- Fixed `$calculate` returning a 500 "Too many concepts" (and the atomic authoring screen silently bouncing back with no product) when authoring an NMPC product whose active ingredient is a modification of two or more base substances, e.g. Lokelma / sodium zirconium cyclosilicate (CUST1730375, NMPC-001949). The `Count of base of active ingredient` ECL in `MedicationProductCalculationService` walked `Is modification of` (738774007) unconditionally and capped the result at the ingredient count, so a single ingredient resolving to two bases exceeded the limit and tripped `SnowstormClient.validatePage`. The walk now gates each modification hop with `: [1..1] 738774007 = *` (only descending through substances that are a modification of exactly one thing) and treats a substance with two or more modification attributes as a base in its own right (`[0..0]` OR `[2..*]`), per the SNOMED editorial guide "Chemical element compound with multiple modification" exception — so such a compound counts as 1 base, matching International Edition concepts like `Product containing only cyanocobalamin (58-Co)`.
- Fixed an incorrect assertion in `MedicationProductCalculationServiceNutritionalNameTest` (introduced alongside the "Product containing only" VMP naming change) that expected the VMPP's FSN-based `owl` axiom to omit the VMP's "Product containing only" prefix. That axiom substitutes the referenced VMP's real FSN, matching every other level's convention (`NameGenerationService#addGeneratedFsnAndPt`), so it legitimately carries the prefix; only the PT-based `pt_owl` axiom is expected to stay prefix-free. No production behaviour changed.
- Deleting a ticket's priority bucket no longer fails with a 500 when the ticket has an associated task; the endpoint now returns an empty `204` instead of serialising the ticket entity (whose lazy `taskAssociation` proxy could not be serialised) (LINGO-NPC-9J).
- Selecting a search result with no concept id no longer sends `undefined` as the product id when loading device/medication product details (which 500'd server-side); the fetch is now skipped unless a concept id is present (SNOMIO-4C).

### Added
- Migrate from Spring Boot 3.3.5 to Spring Boot 4.1.0 (Spring Framework 7.x, Hibernate 7, Spring Security 7, Java 21 compile target). Includes: `ExchangeStrategies` API replaced by `.codecs()` on `WebClient.Builder`; `@MockBean`/`@SpyBean` replaced by `@MockitoBean`; `AntPathRequestMatcher` replaced with direct string patterns in `SecurityConfiguration`; `@EntityScan` package updated to `org.springframework.boot.persistence.autoconfigure`; `spring-boot-starter-aop` renamed to `spring-boot-starter-aspectj`; `RestTemplateBuilder`'s auto-configuration moved out of `spring-boot-autoconfigure` into the new `spring-boot-restclient` module, added as an explicit dependency in `eclrefset`; the injected `WebClient.Builder` bean is likewise used (instead of a bare `WebClient.builder()`) in `DefaultWebClientConfiguration` so Boot's auto-configured codecs (e.g. the Jackson-2 preference below) still apply to the IMS client; `spring.jpa.properties.javax.cache.uri` (Hibernate's second-level cache JCache URI property) is left as-is — Hibernate 7 still reads it unprefixed, and Ehcache's own classpath auto-discovery of `ehcache.xml` picks it up regardless, so the property is effectively inert either way; explicitly wired Spring's own cache abstraction to the same file with `spring.cache.type=jcache` / `spring.cache.jcache.config=classpath:ehcache.xml` — without this, Spring's JCache-backed `@Cacheable`/`@CacheEvict` caches (e.g. `authCookie`) failed to resolve at runtime (`IllegalArgumentException: Cannot find cache named 'authCookie'`) since Spring's cache manager wasn't pointed at the same `ehcache.xml` Hibernate discovers automatically; Lombok upgraded to 1.18.46 with explicit `annotationProcessorPaths`; all Ehcache caches referenced by `@Cacheable`/`@CacheEvict` pre-declared in `ehcache.xml`; Netty version aligned to 4.2.x to match QUIC codec; `snowstorm-java-client` upgraded to `8.1.3-SNAPSHOT` with Spring Boot 4 / Spring Framework 7 compatibility (`UriComponentsBuilder.fromHttpUrl` → `fromUriString`; `HttpHeaders.containsKey` → `containsHeader`; `HttpHeaders.entrySet` → `forEach`; Jackson 2.17.3 modules pinned alongside Spring Boot 4's Jackson 3.x); whitelisted the `CDDL 1.1`/`Eclipse Distribution License - v 1.0` licenses in the root `license-maven-plugin` config, needed for the `javax.xml.bind:jaxb-api`/`com.sun.xml.bind:jaxb-impl` dependencies that `spring-boot-starter-cache`'s eager Ehcache JSR-107 initialisation now requires.

### Fixed
- Fixed Spring `@Cacheable` methods returning `Mono` (`SnowstormClient`/`FhirClient`) failing at runtime after the Spring Boot 4 upgrade. Spring's reactive `@Cacheable` adaptation calls the async `Cache.retrieve(key)` API, which the JCache/Ehcache `Cache` adapter the migration first wired via `spring.cache.type=jcache` does not implement (`UnsupportedOperationException: ... does not support CompletableFuture-based retrieval`). All Spring `@Cacheable`/`@CacheEvict` caches now run on a single Caffeine-backed `CacheManager` (`CacheManagerConfig`), with every cache registered as a Caffeine `AsyncCache` so synchronous and reactive callers are both served by one manager; each cache keeps the TTL/size it previously had in `ehcache.xml`, and `ehcache.xml` is left solely for Hibernate's second-level cache. The now-inert `spring.cache.type=jcache` / `spring.cache.jcache.config` properties are removed (Boot's property-driven cache auto-configuration is disabled once a manual `CacheManager` bean exists).

### Removed
- Removed the dead `h2` Spring profile and its Flyway migration set (`db/migration/h2`), the `com.h2database:h2` dependency, the `latest_ddl.sh` DDL-snapshot script, and the now-orphaned `/api/h2-console/**` security rule and `spring.h2.console` settings. Nothing executed these: the test suite runs against Testcontainers PostgreSQL, CI and the default `local` runtime use PostgreSQL, and the only invoker of the `h2` profile (`latest_ddl.sh`) ran with Flyway disabled. The h2 migration scripts had also drifted from the PostgreSQL set and no longer applied cleanly. (#1912)
...
## [1.3.50] - 2026-07-07
- Fixed the ECL Refset `useSnodineTaskByKey` hook to search across all authoring-platform projects (user tasks + review tasks) instead of the Snomio-configured AP project only, so SNODINE tasks are found correctly.
...
- Corrected nutritional (NMPC) product name generation (IEDC-7476). The VMP is now named from the New Generic Product Name field as entered — the dose form and container are no longer appended a second time (which had duplicated the tail, e.g. "… emulsion for infusion bag emulsion for infusion bag"); the AMP is named from the Branded product name field verbatim (the brand is no longer prepended); and the real-medicinal-product (ATM) is the bare brand with any trailing "(brand)" tag removed (dropping the appended "nutritional product"). Branded product name is now mandatory for nutritional products, since it is used as the AMP name.
- Fixed AMT product calculation erroneously matching an existing MPUU (Clinical Drug) that carries an *additional* active ingredient not present in the product being authored (e.g. a single-ingredient product matching a two-ingredient MPUU). AMT unbranded clinical drugs carry no ingredient-count attribute, and the ingredient role group is generated with ECL "some" semantics, so nothing excluded candidates with extra ingredients — a regression from the #1792 ECL-emission refactor, which moved negative-filter selection to `ModelLevelDefiningAttributes` and dropped the active-ingredient exclusion at the MPUU level. The AMT `CLINICAL_DRUG` level now emits a `[0..0] has-active-ingredient != <modelled ingredients>` exclusion. The fix is AMT-gated (NMPC clinical drugs are already guarded by their count-of-base-active-ingredient attribute) and covers only `has active ingredient`; precise ingredient and BoSS are intentionally still not filtered at this level to avoid over-excluding partially-modelled lookups.
...
- CI: build-pipeline speedups on the `highmemory-pool` agents — Trivy now runs a single filesystem scan and derives the table/SARIF outputs by conversion, Jib base-image layers and the UI Node install dir are cached, `--no-transfer-progress` is applied, and the non-release deploy step no longer re-runs the `common`/`auth` tests. Removed the stale `azure-pipelines_bakup.yml` and a dead commented-out e2e job from the pipeline.
- Build: commit a pinned `ui/pnpm-lock.yaml` and switch the UI install to `pnpm install --frozen-lockfile --prefer-offline` for reproducible and faster frontend builds (previously the lockfile was git-ignored, so every build re-resolved the dependency tree). Dropped the redundant `pnpm list` build step.
- Security: removed the unused `@sentry/remix` dependency (only `@sentry/react` is imported). This eliminates the transitive `turbo-stream` package and with it CVE-2026-34077, so no Trivy suppression is needed for it.
- Test infrastructure: `api` integration tests are isolated per-test (each authors on its own Snowstorm branch; reference data is seeded once instead of wiped per test), and the lingo integration suite runs with method-level parallelism against a single shared Snowstorm/Elasticsearch. No production behaviour change.

### Fixed
- eclrefset: switched Snowstorm concept searches from `GET /concepts?ecl=…` to `POST /concepts/search` with the ECL in the JSON request body, eliminating HTTP 414 failures for long ECL expressions. The `removeInactiveConceptQuery` (ECL with `{{C active = 0}}` description filter) is also moved to POST, removing the need for manual percent-encoding. `AddRemoveQueryThread` updated accordingly to carry the POST body and offset as body fields instead of URL query parameters. Removed the `DefaultUriBuilderFactory.NONE` workaround from the `RestTemplate` bean.
- eclrefset: removed the duplicate `ImsService` class (was an exact copy of `auth/service/ImsService`); eclrefset now depends on `lingo-auth` for IMS authentication, with `spring-boot-starter-security`, `spring-boot-starter-web`, and `spring-boot-starter-webflux` excluded to avoid activating unwanted auto-configuration in the CLI process.
