...
## [Unreleased]
- Renamed the `Nutritional` option in the Target Model dropdown to `Nutritional/Consumer Health`, as requested by HSE. This is a display label only: the stored discriminator value stays `nutritional`, so existing products, saved drafts and the `oneOf` branches that key off it are unaffected. NMPC only — the AMT schema has no nutritional variant. The matching label in the variant's `dynamicDiscriminatorOptions` is updated too; nothing renders it today (the validation helper reads only the `value`), but leaving it saying `Nutritional` would strand a stale copy of the same label. Two other user-visible strings still read "Nutritional" and were deliberately left alone as they are not the Target Model dropdown: the `Nutritional Product` and `Nutritional No Strength` titles in the medication schema, the latter being an option in the per-product strength dropdown. (RITM0509508)
