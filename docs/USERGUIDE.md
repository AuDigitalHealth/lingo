...
The document assumes that you have access to AMT, or some other extension that models medicinal content in the same manner. Snomio in it's current state would not work correctly if you didn't - you would only be able to perform the ticket & task management use cases.

# Snomio

Snomio is a customised tool to manage Medicines Terminology
content requests, author medicines terminology, and manage complex
...
Snomio is the same login as your access to the authoring platform.
...
Snomio includes a ticket management system. Each ticket ideally represents some form of content to be authored - but it could be general workflow items. Users can manually create and update tickets.

Once logged in, a dashboard is displayed. There are two main components
of ticket management on the dashboard view:

a.  **Backlog**: This section contains work tickets generated by the team. The
...
b.  **Backlog Tables:** A condensed view of the work tickets with
...
1.  Click on the ***Backlog*** icon on the banner bar to load the work ticket.
...
6.  To promote the changes made to the ECL query:
    ...
    ii. Locate the ECL task under your project that it was
        > created under.
...
reference set), a report will be dispatched to the mailbox.
