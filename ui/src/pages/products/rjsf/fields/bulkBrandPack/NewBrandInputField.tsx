...
const NewBrandInputField: React.FC<FieldProps> = (props) => {
    ...
    const [inputValue, setInputValue] = useState(formData || { brand: undefined, externalIdentifiers: [] });
    ...
    const handleAdd = () => {
        if (inputValue.brand) {
            ...
            const currentBrands = Array.isArray(currentFormData.brands) ? [...currentFormData.brands] : [];
    ...
    const isBrandDuplicate = () => {
        ...
        return allBrands.some((brand) =>
            brand?.brand?.conceptId === inputValue.brand.conceptId
        );
    };
