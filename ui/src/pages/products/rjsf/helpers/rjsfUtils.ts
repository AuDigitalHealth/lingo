...
export class RjsfUtils {
    ...
    public static rjsfIdToFormDataPath(rjsfId: string): string {
        ...
        return path.replace(/_(\d+)/g, '[$1]').replace(/\.\B/g, '').replace(/_/g, '.');
    }
    ...
    public static rjsfIdToUiSchemaPath(rjsfId: string): string {
        ...
        // Handles cases like 'array_0' -> 'array[0]'
        // The second replace cleans up potential dots left before brackets
        return path.replace(/_(\d+)/g, '_items').replace(/\.\B/g, '').replace(/_/g, '.');
    }
     ...
     */
    public static formDataPathToRjsfId(formDataPath: string | Array<string | number>): string {
        // If the path is an array, join it into a dot-notation string first
        const pathStr = _.isArray(formDataPath) ? formDataPath.join('.') : formDataPath;
     ...
     */
    public static resolveRelativeIdOrPath(idOrPath: string, relativePath: string): string {
     ...
     */
    public static setFormDataById(rootUiSchema: any, rjsfId: string, value: any): any {
        const lodashPath = RjsfUtils.rjsfIdToFormDataPath(rjsfId);
