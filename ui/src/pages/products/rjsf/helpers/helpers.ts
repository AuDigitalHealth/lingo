 ...
 */
export const rjsfIdToLodashPath = (rjsfId: string): string => {
  if (!rjsfId) return '';
  const path = rjsfId.replace(/^root_/, '');
  const parts = path.split('_');
  const converted = parts
    .map(part => {
      return /^\d+$/.test(part) ? `[${part}]` : `.${part}`;
    })
    .join('');
...
export function defaultForCheckBox(value: boolean | undefined) {
  return value ? 'On' : 'Off';
}
export function hasConceptId(v: any) {
  return v?.conceptId ? true : false;
}
export function hasAdditionalFields(v: any) {
  return v?.additionalFields ? true : false;
}
