...
public final class LogThresholdInfo {
  ...
  public static final String ACTION_HAS_NOT_BEEN_CARRIED_OUT_MESSAGE =
  ...
  public static final String ACTION_HAS_BEEN_CARRIED_OUT_MESSAGE =
      "As you have chosen to IGNORE this warning, this action HAS been carried out.";
  public static final String COUNT_THRESHOLD_MESSAGE =
      "ERROR: %d has exceeded the COUNT threshold of %d for refset %s while attempting to %s concepts";
  private static final int THRESHOLD_EXCEEDED_ERROR_CODE = 1;

  private LogThresholdInfo() {}

  /** returns 1 if threshold exceeded, otherwise 1 */
  public static int logAdd(
      String refSetId,
      Integer addCount,
      Integer totalCount,
      ...
      ResultDto resultDto) {
    return log(
        refSetId, "add", addCount, totalCount, percentChangeThreshold, fileAppender, resultDto);
  }

  /** returns 1 if threshold exceeded, otherwise 1 */
  public static int logRemove(
      String refSetId,
      Integer removeCount,
      Integer totalCount,
      double percentChangeThreshold,
  ...
  private static int log(
    ...
    if (count > 0) {
      ...
      if (totalCount.intValue() == 0) {
        log.info(
            "### INFO: no pre-existing content for reference set, skipping threshold calculation");
        ...
        if (resultDto.getNotification() != null) {
          resultDto
              .getNotification()
              .setDescription(
                  resultDto.getNotification().getDescription()
                      + ". "
                      + warningMessage
                      + ". "
                      + carriedOutMessage);
        ...
        // store all the details in a file that will get emailed to notify of the percent threshold
        // been exceeded
        fileAppender.appendToFile(
            "### WARNING: Attempting to "
                ...
                + " members for refset "
                + refSetId
                + " has exceeded the PERCENT threshold of "
                + percentChangeThreshold
                + ".");
        fileAppender.appendToFile(
            "### This action has been carried out, this is just a notification.");
        returnCode = THRESHOLD_EXCEEDED_ERROR_CODE;
      } else {
        log.info(
            "### INFO: "
                + count
