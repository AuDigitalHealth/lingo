...
public class ValidDescriptionValidation implements ConstraintValidator<ValidDescription, String> {
  ...
  /**
   * Initializes the regex pattern after dependency injection is complete. Logs a warning if no
   * regex is provided, indicating that all strings will be considered valid.
   */
  @PostConstruct
  private void initPattern() {
    if (descriptionValidationRegex == null || descriptionValidationRegex.trim().isEmpty()) {
      ...
      try {
        ...
        log.info(
            "Description validation pattern initialized with regex: {}",
            descriptionValidationRegex);
        validationActive = true;
      } catch (PatternSyntaxException e) {
        throw new IllegalArgumentException(
            "Invalid regex in description.validation.regex property.", e);
      }
    }
  }
  ...
  public boolean isValid(String value, ConstraintValidatorContext context) {
    ...
    String messageTemplate =
        (fieldName == null || fieldName.isEmpty() ? "" : fieldName + ": ")
            + "Invalid Concept Description. Value: '"
            + value
   ...
   * @return True if validation is inactive or if the string is a valid description, false
   *     otherwise.
   */
